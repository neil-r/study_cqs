...
class DatabaseSqlLite:
    ...
    def add_model_entry(self, model_id: str, model_name: str, model_size: int, model_version: float):
        try:
    ...
    def has_model(self, model_id):
        try:
            with sqlite3.connect(self.db_file_path) as c:
                o = c.execute("SELECT * FROM model WHERE model_id = ?", (model_id,)).fetchone()
    ...
    def add_task_result(self, task, log, prompt_strategy: str, model_id: str, answer_value: str, discussion_duration: float, answer_response: typing.Optional[str] = None, correct: bool = False):
        try:
            with sqlite3.connect(self.db_file_path) as c:
                cur = c.cursor()
    ...
    def has_task_result(self, task, prompt_strategy, model_id):
        try:
            with sqlite3.connect(self.db_file_path) as c:
                task_content = task.to_json()
                task_json_str = json.dumps(task_content, sort_keys=True)
                o = c.execute("SELECT * FROM task_results WHERE task_id = ? AND prompt_strategy = ? AND model_id = ?", (
                    convert_to_id(task_json_str + prompt_strategy + model_id),
                    prompt_strategy,
                    model_id
                )).fetchone()
    ...
    def get_task_result(self, number):
        ...
        try:
            offset = number - 1
            with sqlite3.connect(self.db_file_path) as c:
                results = c.execute("SELECT * FROM task_results WHERE task_id IN (SELECT task_id FROM task_results ORDER BY task_id LIMIT ?, 1)", (offset,)).fetchmany()
                if results:
                    return [TaskResult(o[0], json.loads(o[1]), json.loads(o[2]), o[3], o[4], o[5], o[6], o[7], o[8] == 1) for o in results]
                else:
                    return None
        except sqlite3.Error as e:
