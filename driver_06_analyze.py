    ...
    df_results = pd.read_sql('''
        SELECT model_id, answer_value as has_events, text_filter_correct_result, classification_correct_result, extraction_correct_result, COUNT(*) as instance_count FROM (
            ...
            FROM task_results as tr INNER JOIN (
                SELECT * FROM task_results WHERE prompt_strategy == "full_text_search"
            ) as ftr ON ftr.task = tr.task
...
for model_id in model_ids:
    ...
    print(f"truth | {df_true[(df_true['text_filter_correct_result'] == 1) & (df_true['extraction_correct_result'] == 1)]['instance_count'].sum()} | {df_true[(df_true['extraction_correct_result'] == 0) | (df_true['text_filter_correct_result'] == 0)]['instance_count'].sum()}")
    print(f"false | {df_false[(df_false['extraction_correct_result'] == 0) & (df_false['text_filter_correct_result'] == 0)]['instance_count'].sum()} | {df_false[(df_false['extraction_correct_result'] == 1) | (df_false['text_filter_correct_result'] == 1)]['instance_count'].sum()}")

    print("C+T Confusion Matrix")
    print(f"truth | {df_true[(df_true['classification_correct_result'] == 1) & (df_true['text_filter_correct_result'] == 1)]['instance_count'].sum()} | {df_true[(df_true['text_filter_correct_result'] == 0) | (df_true['classification_correct_result'] == 0)]['instance_count'].sum()}")
    print(f"false | {df_false[(df_false['text_filter_correct_result'] == 0) & (df_false['classification_correct_result'] == 0)]['instance_count'].sum()} | {df_false[(df_false['text_filter_correct_result'] == 1) | (df_false['classification_correct_result'] == 1)]['instance_count'].sum()}")

    print("All Confusion Matrix")
    print(f"truth | {df_true[(df_true['text_filter_correct_result'] == 1) & (df_true['classification_correct_result'] == 1) & (df_true['extraction_correct_result'] == 1)]['instance_count'].sum()} | {df_true[(df_true['text_filter_correct_result'] == 0) | (df_true['extraction_correct_result'] == 0) | (df_true['classification_correct_result'] == 0)]['instance_count'].sum()}")
    print(f"false | {df_false[(df_false['text_filter_correct_result'] == 0) & (df_false['extraction_correct_result'] == 0) & (df_false['classification_correct_result'] == 0)]['instance_count'].sum()} | {df_false[(df_false['text_filter_correct_result'] == 1) | (df_false['extraction_correct_result'] == 1) | (df_false['classification_correct_result'] == 1)]['instance_count'].sum()}")
